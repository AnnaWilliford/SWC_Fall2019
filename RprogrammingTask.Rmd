...
Here are the steps I need to implement in R to complete the above task.  
1. Load ggplot2 library  
2. Load gapminder.txt into R  
3. Create `country_list` that contains country names from `gapminder.txt`. 
4. Start a `for` loop to iterate over every elemet of `country_list`
...
7. Make ggplot to plot life expectancy on y-axis and years on x-axis.  
8. End `for` loop.
...
```
Here is the implementations of these steps:  
...
If you want to save plots to file, make a unique name for every plot and save with `ggsave`
